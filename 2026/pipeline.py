...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.features_cache is None:
            self.features_cache = f"{self.artifact_dir}/features_cache.npz"
        if self.svm_clf_path is None:
            self.svm_clf_path = f"{self.artifact_dir}/svm_clf.pkl"
        if self.scaler_path is None:
            self.scaler_path = f"{self.artifact_dir}/scaler.pkl"
        if self.pca_path is None:
            self.pca_path = f"{self.artifact_dir}/pca.pkl"
        if self.val_paths_cache is None:
            self.val_paths_cache = f"{self.artifact_dir}/val_paths.txt"
        if self.hnm_train_final is None:
            self.hnm_train_final = f"{self.artifact_dir}/hnm_train_final.npz"
...
def crop_centered(
    img: np.ndarray, bbox: dict, window_size: int, image_size: int
) -> np.ndarray:
...
def crop_grid(
    img: np.ndarray, col: int, row: int, window_size: int, image_size: int
) -> np.ndarray:
...
def preprocess_all_images(cfg: Config) -> tuple:
    ...
    for folder in (
        cfg.microglia_folder,
        cfg.noise_folder,
        cfg.microglia_val_folder,
        cfg.noise_val_folder,
        cfg.microglia_test_folder,
        cfg.noise_test_folder,
    ):
        clear_folder(folder)
    ...
    print(
        f"Source — train: {len(train_paths)}, val: {len(val_paths)}, "
    ...
    for path in train_paths:
        ...
        extract_patches_from_image(
            path, cfg.microglia_folder, cfg.noise_folder, cfg, augment=True
        )
    ...
    for path in val_paths:
        print(f"  {os.path.basename(path)}")
        extract_patches_from_image(
            path, cfg.microglia_val_folder, cfg.noise_val_folder, cfg, augment=False
        )
...
def bin_spatial(img: np.ndarray, size: tuple) -> np.ndarray:
    ...
    return np.hstack(
        [cv2.resize(img[:, :, ch], size).ravel() for ch in range(img.shape[2])]
    )
...
def color_hist(img: np.ndarray, nbins: int) -> np.ndarray:
    ...
    return np.concatenate(
        [np.histogram(img[:, :, ch], bins=nbins)[0] for ch in range(img.shape[2])]
    )


def lbp_features(
    img_gray: np.ndarray, radius: int, n_points: int, n_bins: int
) -> np.ndarray:
...
def _hog_for_channels(
    img: np.ndarray, orient: int, ppc: int, cpb: int, hog_channel: str
) -> np.ndarray:
...
def _feature_vec_full(raw_rgb: np.ndarray, cfg: Config) -> np.ndarray:
    ...
    hog_fine = _hog_for_channels(
        img,
        cfg.hog_orient,
        cfg.hog_pix_per_cell_fine,
        cfg.hog_cell_per_block,
        cfg.hog_channel,
    )
    hog_coarse = _hog_for_channels(
        img,
        cfg.hog_orient,
    ...
    * ``'full'``       — spatial bins + colour hist + LBP + intensity stats +
    ...
    for i, path in enumerate(image_paths):
        ...
        try:
            ...
            # Cast to float32 at source. skimage's hog() returns float64; without
            # this each vector doubles in RAM and np.vstack's transient peak on
            # the full feature space (~33k-d × 15k samples) crosses the OOM line
            # (see 2026-05-23 OOM diagnosis).
...
def extract_raw_features(cfg: Config):
    ...
    print(
        f"Train    — microglia: {len(train_pos_paths)}, noise: {len(train_neg_paths)}"
    )
    ...
    # Consolidate eagerly + free the source lists so the transient peak during
    # vstack doesn't stack list + result + astype-copy simultaneously.
    import gc

    X_train_raw = np.vstack((train_pos_feats, train_neg_feats)).astype(
    ...
    X_val_raw = np.vstack((val_pos_feats, val_neg_feats)).astype(np.float32, copy=False)
    y_val_raw = np.hstack((np.ones(len(val_pos_feats)), np.zeros(len(val_neg_feats))))
    del val_pos_feats, val_neg_feats
    gc.collect()
...
def fit_pipeline(
    ...
    if cfg.pca_n_components > 0:
        n_components = min(
            int(cfg.pca_n_components), X_train.shape[1], X_train.shape[0]
        )
        pca = PCA(
        ...
        print(
            f"PCA: {X_train_raw.shape[1]} → {pca.n_components_} components "
...
def evaluate_classifier(
    clf: LinearSVC, X: np.ndarray, y: np.ndarray, label: str = ""
) -> None:
...
def sliding_window_detect(
    ...
    if full_mode:
        ...
        hog_maps_f = [
            hog_features(
                img_c[:, :, ch], cfg.hog_orient, ppc_f, cpb, feature_vector=False
            )
            for ch in range(n_ch)
        ]
    ...
    for xb in range(nxsteps):
        ...
        for yb in range(nysteps):
            ...
            hog_coarse = np.hstack(
                [
                    hm[
                        ypos_c : ypos_c + nblocks_win_c, xpos_c : xpos_c + nblocks_win_c
                    ].ravel()
            ...
            if full_mode:
                ...
                hog_fine = np.hstack(
                    [
                        hm[
                            ypos_f : ypos_f + nblocks_win_f,
                            xpos_f : xpos_f + nblocks_win_f,
                        ].ravel()
                ...
                lbp_feat = np.histogram(
                    ...
                    bins=cfg.lbp_n_bins,
                    range=(0, cfg.lbp_n_points + 2),
                    density=True,
                )[0].astype(np.float32)
                int_st = _intensity_stats_fast(
                    integ_sum, integ_sq, integ_cu, ytop, xleft, win, n_ch
                )
                sub = img_c[ytop : ytop + win, xleft : xleft + win]
                spatial = bin_spatial(sub, cfg.spatial_size)
                chist = color_hist(sub, cfg.hist_bins)
                feat = np.hstack(
                    (spatial, chist, lbp_feat, int_st, hog_fine, hog_coarse)
                )
            else:
                feat = hog_coarse
...
def non_max_suppression(detections: list, scores: list, iou_thresh: float) -> list:
    ...
    if not detections:
        return []
    boxes = np.array(
        [[x0, y0, x1, y1] for (x0, y0), (x1, y1) in detections], dtype=float
    )
...
def evaluate_detections(
    pred_boxes: list, gt_boxes: list, iou_thresh: float = 0.5
) -> dict:
...
def plot_gt_vs_pred(
    img: np.ndarray, pred_boxes: list, gt_boxes: list, title: str = ""
) -> None:
...
def detection_level_test_eval(
    ...
    print(
        f"Running detection on {len(final_test_paths)} test images "
    ...
    for path in final_test_paths:
        ...
        if gt_boxes:
            ...
            total_fn += m["fn"]
            print(
                f"  GT={len(gt_boxes)}  Pred={len(boxes)}  "
